...
for i in $(seq 1 $EC2_COUNT); do
  echo "Launching EC2 Instance $i in $AZ..."
  ...
  aws ec2 create-tags --resources $INSTANCE_ID --tags Key=Name,Value=$TAG-EC2-$i
  echo "EC2 Instance $i Launched: $INSTANCE_ID"
...
for i in $(seq 1 $EC2_COUNT); do
  echo "EC2 Instance $i: ${INSTANCE_IDS[$((i-1))]} (IP: ${PUBLIC_IPS[$((i-1))]})"
done
echo "---------------------------------------"
